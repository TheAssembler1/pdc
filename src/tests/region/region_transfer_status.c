...
main(int argc, char **argv)
{
    ...
    while (1) {
        TASSERT(PDCregion_transfer_status(transfer_request, &completed) >= 0,
                "Call to region_transfer_status succeeded", "Call to region_transfer_status failed");
        if (completed == PDC_TRANSFER_STATUS_COMPLETE)
            break;
        else if (completed == PDC_TRANSFER_STATUS_NOT_FOUND)
            TGOTO_ERROR(FAIL, "Transfer request status not found");
        sleep(1);
    }
    ...
    if (completed != PDC_TRANSFER_STATUS_NOT_FOUND)
        TGOTO_ERROR(TFAIL, "Transfer request status wrong value");
    ...
    while (1) {
        ...
        if (completed != PDC_TRANSFER_STATUS_PENDING)
            ...
            TGOTO_ERROR(TFAIL, "Transfer request status not found");
        sleep(1);
    }
