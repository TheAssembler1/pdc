...
main(int argc, char **argv)
{
    ...
    TASSERT((transfer_request =
                 PDCregion_transfer_create(data + BUF_LEN / 2, PDC_WRITE, obj1, reg, reg_global)) != 0,
            "Call to PDCregion_transfer_create succeeded", "Call to PDCregion_transfer_create failed");
    TASSERT(PDCregion_transfer_start(transfer_request) >= 0, "Call to PDCregion_transfer_start succeeded",
            "Call to PDCregion_transfer_start failed");
    TASSERT(PDCregion_transfer_wait(transfer_request) >= 0, "Call to PDCregion_transfer_wait succeeded",
    ...
    for (i = 0; i < BUF_LEN / 2; ++i) {
        if (data_read[i] != i + BUF_LEN / 4)
            TGOTO_ERROR(TFAIL, "i = %d, wrong value %d!=%d", i, data_read[i], i + BUF_LEN / 4);
    }
    ...
    TASSERT((transfer_request =
                 PDCregion_transfer_create(data + BUF_LEN / 8, PDC_WRITE, obj1, reg, reg_global)) != 0,
            "Call to PDCregion_transfer_create succeeded", "Call to PDCregion_transfer_create failed");
    TASSERT(PDCregion_transfer_start(transfer_request) >= 0, "Call to PDCregion_transfer_start succeeded",
            "Call to PDCregion_transfer_start failed");
    TASSERT(PDCregion_transfer_wait(transfer_request) >= 0, "Call to PDCregion_transfer_wait succeeded",
    ...
    for (i = 0; i < BUF_LEN / 2; ++i) {
        if (data_read[i] != i + BUF_LEN / 8 + BUF_LEN)
            TGOTO_ERROR(TFAIL, "i = %d, wrong value %d!=%d", i, data_read[i], i + BUF_LEN / 8 + BUF_LEN);
    }
