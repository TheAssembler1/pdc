 ...
 * -----------------------------------------------------------------------------
 *
 * Verifies the AN framework's dynamic CPU/GPU variant selection end to end.
 * Uses vector_magnitude_dynamic.json, which omits "device" on its
 * "vector_magnitude" transformation -- so PDCan_dg_json_create_common
 * resolves it to two variants (CPU and GPU), and PDCan_exec_graph's
 * an_select_variant (pdc_an_server.c) picks between them at execution time.
 *
 * Inputs are PDC_DOUBLE (unlike vector_magnitude_test.c's PDC_FLOAT), since
 * the first-cut GPU kernel (pdc_an_builtin_magnitude_gpu.cu) only supports
 * double-precision input.
 *
 * The server process controls which variant actually runs via the
 * PDC_AN_FORCE_DEVICE env var (read server-side by an_select_variant) --
 * this test itself makes no device selection; running it once against a
 * server started with PDC_AN_FORCE_DEVICE=GPU and once with
 * PDC_AN_FORCE_DEVICE=CPU (or unset, for the real dynamic path) verifies
 * both variants independently produce the same correct result.
 */
...
main(int argc, char **argv)
{
    ...
    double *vx_data  = (double *)malloc(sizeof(double) * BUF_LEN);
    double *vy_data  = (double *)malloc(sizeof(double) * BUF_LEN);
    double *vz_data  = (double *)malloc(sizeof(double) * BUF_LEN);
    double *mag_data = (double *)malloc(sizeof(double) * BUF_LEN);

    for (i = 0; i < BUF_LEN; ++i) {
        vx_data[i] = (double)i;
        vy_data[i] = (double)(i + 1);
        vz_data[i] = (double)(i + 2);
    }
    ...
    // Input objects (vx, vy, vz) are PDC_DOUBLE -- required by the GPU variant
    TASSERT((obj_prop_in = PDCprop_create(PDC_OBJ_CREATE, pdc)) != 0, "Call to PDCprop_create succeeded",
            "Call to PDCprop_create failed");
    TASSERT(PDCprop_set_obj_type(obj_prop_in, PDC_DOUBLE) >= 0, "Call to PDCprop_set_obj_type succeeded",
    ...
    TASSERT(PDCprop_set_obj_app_name(obj_prop_in, "AnalysisGpuTest") >= 0,
    ...
    TASSERT(PDCprop_set_obj_time_step(obj_prop_out, 0) >= 0, "Call to PDCprop_set_obj_time_step succeeded",
            "Call to PDCprop_set_obj_time_step failed");
    TASSERT(PDCprop_set_obj_app_name(obj_prop_out, "AnalysisGpuTest") >= 0,
            "Call to PDCprop_set_obj_app_name succeeded", "Call to PDCprop_set_obj_app_name failed");
    ...
    // Attach the dynamic-device analysis graph: vx/vy/vz are inputs, magnitude is the output
    TASSERT((dg_id = PDCan_dg_json_create(AN_GRAPHS_DIR "vector_magnitude_dynamic.json")) != 0,
    ...
    // Read magnitude: not materialized yet, so the server transparently
    // computes it via whichever variant an_select_variant (or
    // PDC_AN_FORCE_DEVICE, if the server was started with it set) chooses.
    ...
    for (i = 0; i < BUF_LEN; ++i) {
        double expected = sqrt(vx_data[i] * vx_data[i] + vy_data[i] * vy_data[i] + vz_data[i] * vz_data[i]);
        if (fabs(mag_data[i] - expected) > EPSILON)
            TGOTO_ERROR(TFAIL, "magnitude[%d] = %f, expected %f", i, mag_data[i], expected);
    }
    LOG_INFO("Dynamic-device magnitude read matched expected values\n");
