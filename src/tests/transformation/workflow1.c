/**
 * Description: This workflow creates a simple DG that has a
 * compression transformation and a decompression transformation.
 * It then attaches the created DG to an object region which is
 * then read from and written to. When read into client memory
 * the data region should be decompressed. When written to the
 * server the data region should be compressed.
 */
...
workflow1(pdcid_t pdc, pdcid_t cont)
{
    ...
    pdcid_t compressed_id   = PDCtf_create_state("compressed");
    pdcid_t decompressed_id = PDCtf_create_state("decompressed");

    pdcid_t comp_func_id = PDCtf_create_func("lib:compr", PDC_TF_CPU_DEVICE, decompressed_id, compressed_id);
    pdcid_t decomp_func_id =
        PDCtf_create_func("lib:decompr", PDC_TF_CPU_DEVICE, compressed_id, decompressed_id);
    ...
    PDCtf_attach_to_region(dg_id, obj_id, reg, decompressed_id, compressed_id);
