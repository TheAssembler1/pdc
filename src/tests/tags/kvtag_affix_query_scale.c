...
main(int argc, char *argv[])
{
    ...
    if (my_rank == 0) {
        LOG_INFO("Each client will create about %d obj\n", my_obj);
    }
    ...
    if (my_rank == 0)
        LOG_INFO("All clients created %d objects\n", n_obj);
    ...
    for (i = 0; i < my_obj_after_selectivity; i++) {
        for (iter = 0; iter < round; iter++) {
            ...
            free(kvtag.name);
            free(kvtag.value);
            // TODO: Check correctness of the query results.
        }
        // TODO: why n_obj has to be larger than 1000?
        if (my_rank == 0 /*&& n_obj > 1000 */)
            LOG_INFO("Rank %d: Added %d kvtag to the %d / %d th object, I'm applying selectivity %d to %d "
                     "objects.\n",
    ...
    if (my_rank == 0) {
        LOG_INFO("[TAG Creation] Rank %d: Added %d kvtag to %d objects, time: %.5f ms\n", my_rank, round,
    ...
    for (comm_type = 1; comm_type >= 0; comm_type--) {
        for (query_type = 0; query_type < 4; query_type++) {
            ...
            if (my_rank == 0) {
                ...
                LOG_INFO("[%s Client %s Query with%sINDEX] %d rounds with %d results, time: %.5f ms\n",
...
#endif
    if (my_rank == 0) {
        LOG_INFO("[TAG Deletion] Rank %d: Deleted %d kvtag from %d objects, time: %.5f ms\n", my_rank, round,
                 my_obj, total_time * 1000.0);
    }
