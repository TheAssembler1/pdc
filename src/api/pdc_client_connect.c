                            ...
                            pdcid_t *metadata_id, struct _pdc_obj_info *obj_pointer)
{
    ...
    if (obj_pointer != NULL && obj_pointer->pdc_tf_obj != NULL &&
        ...
        char *json_filepath = (char *)pdc_tf_graphs[dg_id]->data;
        ...
        in.store_state   = (hg_string_t)region_mapping->region_state.store_state;
        in.client_state  = (hg_string_t)region_mapping->region_state.client_state;

        if (access_type == PDC_WRITE)
            in.cur_state = (hg_string_t)region_mapping->region_state.client_state;
        else if (access_type == PDC_READ)
            in.cur_state = (hg_string_t)region_mapping->region_state.store_state;
        else
        ...
        in.json_filepath = (hg_string_t)NULL;
        in.store_state   = (hg_string_t)NULL;
        in.cur_state     = (hg_string_t)NULL;
        in.client_state  = (hg_string_t)NULL;
    }
