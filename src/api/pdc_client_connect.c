                            ...
                            pdcid_t *metadata_id, struct _pdc_obj_info *obj_pointer)
{
    ...
    in.remote_unit   = unit;
    in.obj_id        = obj_id;
    in.json_filepath = NULL;
    in.cur_state     = NULL;
    in.stored_state  = NULL;
    ...
    // Check for associated transformation
    if (PDCtf_region_has_attached_graph(obj_pointer, remote_ndim, unit, remote_offset, remote_size,
                                        &region_mapping)) {
        LOG_INFO("Found associated transformation for obj_id %llu", obj_id);
        else
        {
            LOG_INFO("No associated transformation found for obj_id %llu", obj_id);
        }

        hg_ret = HG_Create(send_context_g, pdc_server_info_g[data_server_id].addr,
                           transfer_request_register_id_g, &client_send_transfer_request_handle);
...

    {
        ...
        hg_ret = HG_Forward(client_send_transfer_request_wait_handle,
                            client_send_transfer_request_wait_rpc_cb, &transfer_args, &in);
#ifdef PDC_TIMING
        end = MPI_Wtime();
        if (access_type == PDC_READ) {
    ...
    perr_t PDC_Client_buf_map(pdcid_t local_region_id, pdcid_t remote_obj_id, size_t ndim,
                              uint64_t *local_dims, uint64_t *local_offset, pdc_var_type_t local_type,
                              void *local_data, pdc_var_type_t remote_type,
                              struct pdc_region_info *local_region, struct pdc_region_info *remote_region,
...
close:
    ...
    perr_t PDC_get_io_stats_mpi(double read_time, double query_time, int nfopen)
    {
        ...
        if (pdc_client_mpi_rank_g == 0) {
            LOG_INFO("IO STATS (MIN, AVG, MAX)\n"
                     ...
                     nread_bb_g, read_bb_size_g, read_time_min, read_time_avg, read_time_max,
                     reduce_overhead);
        }
                                                            ...
                                                            size_t *out_buf_sizes)
    {
        ...
        if (pdc_client_same_node_rank_g == 0) {
            ...
            // ntotal_obj);
            res_storage_meta_1d =
                (region_storage_meta_t *)PDC_malloc(sizeof(region_storage_meta_t) * ntotal_obj);
            for (i = 0; i < ntotal_obj; i++)
                        ...
                        *total_read_bytes += read_bytes;
                        if (is_client_debug_g == 1) {
                            LOG_INFO("z: %" PRIu64 ", j: %" PRIu64 ", Read data size=%" PRIu64 ": [%.*s]\n",
                                     j, i, overlap_count[0], (int)overlap_count[0],
    ...
    static perr_t PDCtag_delete(pdcid_t obj_id, char *tag_name, int is_cont)
    {
        FUNC_ENTER(NULL);
    ...
    void report_avg_server_profiling_rst()
    {
        ...
        for (int i = 0; i < pdc_server_num_g; i++) {
            ...
            LOG_INFO("Server %d, avg profiling time: %.4f ms, memory usage: %.4f MB\n", i,
                     avg_srv_time / 1000.0, srv_mem_usage);
        }
    ...
    static hg_return_t client_dart_get_server_info_cb(const struct hg_cb_info *callback_info)
    {
        FUNC_ENTER(NULL);
    ...
    void dart_retrieve_server_info_cb(dart_server * server_ptr)
    {
        FUNC_ENTER(NULL);
    ...
    // Bulk
    static hg_return_t dart_perform_one_server_on_receive_cb(const struct hg_cb_info *callback_info)
    {
        FUNC_ENTER(NULL);
        ...
        /* Pull bulk data */
        ret_value = HG_Bulk_transfer(hg_info->context, hg_test_bulk_transfer_cb, client_lookup_args,
    ...
    perr_t _dart_send_request_to_one_server(int server_id, dart_perform_one_server_in_t *dart_in,
    ...
    int _aggregate_dart_results_from_all_servers(struct bulk_args_t * lookup_args, Set * output_set,
                                                 int num_requests)
    {
        FUNC_ENTER(NULL);
    ...
    uint64_t dart_perform_on_servers(index_hash_result_t * *hash_result, int num_servers,
...
// #define ENABLE_MPI
    ...
    void get_first_sender(int *first_sender_global_rank, int *num_groups, int *sender_group_id,
                          int *rank_in_group, int *sender_group_size, int *prefer_custom_exchange)
    {
        FUNC_ENTER(NULL);
    ...
    void PDC_assign_server(int32_t *my_server_start, int32_t *my_server_end, int32_t *my_server_count)
    {
        FUNC_ENTER(NULL);
    ...
    void _standard_all_gather_result(int query_sent, int *n_res, uint64_t **pdc_ids, MPI_Comm world_comm)
    {
        FUNC_ENTER(NULL);
        ...
        MPI_Allgatherv(*pdc_ids, *n_res, MPI_UINT64_T, all_ids, all_nmeta_array, disp, MPI_UINT64_T,
                       world_comm);
    ...
    void _customized_all_gather_result(int query_sent, int *n_res, uint64_t **pdc_ids, MPI_Comm world_comm)
    {
        FUNC_ENTER(NULL);
    ...
    void _standard_bcast_result(int root, int *n_res, uint64_t **out, MPI_Comm world_comm)
    {
        FUNC_ENTER(NULL);
    ...
    void _customized_bcast_result(int first_sender_global_rank, int num_groups, int sender_group_id,
                                  int rank_in_group, int sender_group_size, int *n_res, uint64_t **out,
                                  MPI_Comm world_comm)
    {
        FUNC_ENTER(NULL);
    ...
    perr_t PDC_Client_search_obj_ref_through_dart_mpi(dart_hash_algo_t hash_algo, char *query_string,
                                                      ...
                                                      uint64_t **out, MPI_Comm world_comm)
    {
        FUNC_ENTER(NULL);
