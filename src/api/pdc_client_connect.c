                            ...
                            pdcid_t *metadata_id, struct _pdc_obj_info *obj_pointer)
{
    ...
    if (obj_pointer != NULL && obj_pointer->pdc_tf_obj != NULL &&
        ...
        in.store_state   = region_mapping->region_state.store_state;
        in.client_state  = region_mapping->region_state.client_state;

        if (access_type == PDC_WRITE)
            in.cur_state = region_mapping->region_state.client_state;
        else if (access_type == PDC_READ)
            in.cur_state = region_mapping->region_state.store_state;
        else
