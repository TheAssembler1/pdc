                            ...
                            pdcid_t *metadata_id, struct _pdc_obj_info *obj_pointer)
{
    ...
    if (access_type == PDC_WRITE &&
        PDCtf_region_has_attached_graph(obj_pointer, remote_ndim, unit, remote_offset, remote_size,
                                        &region_mapping)) {
        ...
        in.cur_state     = region_mapping->region_state.cur_state;
        in.stored_state  = region_mapping->region_state.desired_state;
    }
    else {
