...
hg_proc_transfer_request_in_t(hg_proc_t proc, void *data)
{
    ...
    ret = hg_proc_hg_string_t(proc, &struct_data->cur_state);
    if (ret != HG_SUCCESS) {
        FUNC_LEAVE(ret);
    }
    ret = hg_proc_hg_string_t(proc, &struct_data->client_state);
    if (ret != HG_SUCCESS) {
        FUNC_LEAVE(ret);
    }
    ret = hg_proc_hg_string_t(proc, &struct_data->store_state);
    if (ret != HG_SUCCESS) {
        FUNC_LEAVE(ret);
    }
