...
HG_TEST_RPC_CB(transfer_request, handle)
{
    ...
    ret_value = HG_Respond(handle, NULL, NULL, &out);
    if (in.access_type == PDC_WRITE) {
        if (in.json_filepath != NULL && strlen(in.json_filepath) > 0) {
            ...
            if (PDCtf_store_json_mapping(in.obj_id, in.json_filepath, in.cur_state, in.client_state,
                                         in.store_state, in.remote_region.start, in.remote_region.count,
