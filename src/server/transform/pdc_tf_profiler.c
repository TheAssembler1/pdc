...
static perr_t pdc_tf_nvml_profiler_update() {
    FUNC_ENTER(NULL);
    ...
    // We should not have to malloc too much sense every time we update the profiler we can just overwrite the existing samples in the array, 
    // but we need to malloc once to create the array of samples for each device. But this is a FIXME
    pdc_tf_profiler_nvml_sample_t* nvml_samples_array = (pdc_tf_profiler_nvml_sample_t*)PDC_malloc(pdc_tf_profiler_nvml_device_count * sizeof(pdc_tf_profiler_nvml_sample_t));
    ...
    for (int i = 0; i < pdc_tf_profiler_nvml_device_count; i++) {
        ...
        if (nvml_ret != NVML_SUCCESS) {
            ...
            LOG_INFO("NVML Device %d Memory: Total: %lu MB, Used: %lu MB, Free: %lu MB\n", 
                    i, mem_info.total / (1024 * 1024), mem_info.used / (1024 * 1024), mem_info.free / (1024 * 1024));
        }
...
perr_t pdc_tf_update_profiler() {
    ...
    if(!pdc_tf_profiler_init) {
        ...
        pdc_tf_profiler_samples.nvml_samples = pdc_vector_create(DEFAULT_PROFILER_SAMPLES_CAPACITY, DEFAULT_PROFILER_SAMPLES_EXPANSION_FACTOR);
    }
