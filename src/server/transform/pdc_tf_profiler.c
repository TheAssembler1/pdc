...
pdc_tf_nvml_profiler_update()
{
    ...
    for (int i = 0; i < pdc_tf_profiler_nvml_device_count; i++) {
        ...
        // Log utilization
        LOG_INFO("Device %d - GPU Util: %f, Mem Util: %u%%, Mem Total: %lu MB, Mem Used: %lu MB, Mem Free: %lu MB\n",
                 i,
                 nvml_sample[i].gpu_utilization,
                 nvml_sample[i].memory_utilization,
                 nvml_sample[i].memory_total / (1024 * 1024),
                 nvml_sample[i].memory_used / (1024 * 1024),
