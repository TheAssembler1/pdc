...
HG_TEST_RPC_CB(buf_unmap, handle)
{
    ...
    PDC_TIMING_DECLARE(PDC_SERVER_BUF_OBJ_UNMAP_RPC_TIME);

    FUNC_ENTER(NULL);

    PDC_TIMING_START(PDC_SERVER_BUF_OBJ_UNMAP_RPC_TIME);
...
HG_TEST_RPC_CB(buf_map, handle)
{
    ...
    void                 *data_ptr;
    size_t                ndim;
    PDC_TIMING_DECLARE(PDC_SERVER_BUF_OBJ_MAP_RPC_TIME);
    ...
    PDC_TIMING_END(PDC_SERVER_BUF_OBJ_MAP_RPC_TIME);
done:
    fflush(stdout);
    FUNC_LEAVE(ret_value);
}
