...
int
set_hdf5_method(int hdf5_method)
{
...
static int
wrap_hdf5_spaces(int total_requests, hsize_t *start, hsize_t *end, hid_t did, hid_t *dsid_ptr,
                 hid_t *msid_ptr)
{
...
#ifdef PDC_PATCH
int
register_multidataset_request(const char *name, void *buf, hsize_t start, hsize_t end, hid_t mtype)
{
#else
int
register_multidataset_request(const char *name, hid_t gid, void *buf, hsize_t start, hsize_t end, hid_t mtype)
{
#endif
    std::string                                           s(name);
    std::map<std::string, multidataset_array *>::iterator it;
...
int
register_multidataset_request_append(const char *name, hid_t gid, void *buf, hsize_t data_size, hid_t mtype)
{
    std::string s(name);
    hsize_t     start, end;
...
static int
merge_requests(std::vector<hsize_t> *start, std::vector<hsize_t> *end, int request_size,
               std::vector<char *> *buf, hsize_t **new_start, hsize_t **new_end, char **new_buf, hid_t mtype,
               int *request_size_ptr)
{
    int    i, index;
...
#endif
    i = 0;
    LOG_DEBUG("Number of datasets to be written %d\n", rank, dataset_size);
...
// MPI_Barrier(MPI_COMM_WORLD);
        ...
        merge_requests(it->second->start, it->second->end, it->second->start->size(), it->second->temp_mem,
                       &new_start, &new_end, &(temp_buf[i]), it->second->mtype, &new_request_size);
        ...
        wrap_hdf5_spaces(new_request_size, new_start, new_end, it->second->did,
                         &(multi_datasets_temp[i].dset_space_id), &(multi_datasets_temp[i].mem_space_id));
        i++;
    }
    ...
    for (it = multi_datasets.begin(); it != multi_datasets.end(); ++it) {
        ...
        for (j = 0; j < new_request_size; ++j) {
            ...
            pdcid_t transfer_request_id =
                PDCregion_transfer_create(ptr, PDC_WRITE, it->second->did, reg, reg);
            LOG_DEBUG("did = %lu, starting request offset = %lu, size = %lu, j = %d\n", it->second->did,
                      offset, offset_length, j);
